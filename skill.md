...
1. **NLU 配置驱动**：意图识别完全由 `nlu_config.json` 外部配置驱动，支持 20 个意图，每个意图 4-12 个关键词 + 正则 patterns，新增/修改意图只需编辑配置文件
...
1. **自然语言理解（NLU）**：基于 `nlu_config.json` 配置驱动的意图识别，支持 20 个意图、正则模式匹配、关键词匹配、模糊匹配
...
1. **NLU 解析**：基于 `nlu_config.json` 配置驱动的自然语言理解
   - 正则模式匹配（最高优先级，置信度 0.95）
   - 关键词匹配（置信度最高 0.9）
   - 模糊匹配：SequenceMatcher 相似度计算（阈值 0.6）
   - 置信度计算：综合评分 0-1
   - 支持 20 个意图，每个意图 4-12 个关键词 + 正则 patterns
...
使用 `verify_engine.py` 进行路由逻辑验证（285个断言，覆盖全部16个模块）：
...
该脚本会运行一系列测试用例，验证 NLU 配置驱动的意图识别准确性、日志系统的完整性、路由决策的正确性、崩溃检测的可靠性、双链路通信的稳定性、自动设备注册的准确性和语音通知中继的完整性。
