...
智能调用引擎（SIE）是多设备协作场景下的任务调度技能。它在 v1.x 路由引擎的基础上，新增五大核心能力：
...
4. **自动设备注册**：新设备登录账号后自动发现、探测能力、接入引擎，无需手动配置
5. **语音通知中继**：电脑端完成工作后，自动选择有语音能力的设备播报结果
...
9. **自动设备注册（Auto Device Registration）**：新设备自动发现、能力探测、动态注册
10. **语音通知中继（Voice Notification Relay）**：任务完成后自动选择语音设备播报结果
...
```
任务输入 → 自动设备注册 → 任务分析 → 设备能力匹配 → 负载评估 → 故障检查 → 崩溃风险评估 → 路由决策 → 指示器输出 → 任务分发 → 语音通知
```

### 详细步骤

1. **自动设备注册**：检查是否有新设备上线，自动探测能力并注册
2. **任务分析**：调用 Task Profiler 提取任务特征向量
3. **能力匹配**：将任务特征与各设备能力画像进行匹配，计算匹配分数
4. **负载评估**：查询各设备当前负载，计算可用容量
5. **故障检查**：确认目标设备在线且健康（双链路任一可用即可）
6. **崩溃风险评估**：对目标设备执行 Crash Detector 风险扫描
7. **综合评分**：`score = α × capability_match + β × load_factor + γ × proximity`
   - `α = 0.5`（能力匹配权重）
   - `β = 0.3`（负载均衡权重）
   - `γ = 0.2`（可达性权重，基于心跳延迟分级：<2s→1.0, <5s→0.8, ≥5s→0.5）
8. **路由决策**：选择综合评分最高的设备
9. **指示器输出**：生成 Invocation Indicator，展示决策结果
10. **任务分发**：通过主链路（device_chat）分发任务
11. **语音通知**：任务完成后，选择最佳语音设备播报结果
...
使用 `verify_engine.py` 进行路由逻辑验证（200+个断言，覆盖全部12个模块）：

```bash
python verify_engine.py
```

该脚本会运行一系列测试用例，验证路由决策的正确性、崩溃检测的可靠性、双链路通信的稳定性、自动设备注册的准确性和语音通知中继的完整性。
